...
LINE Bot + OpenAI + Supabase を組み合わせた株式投資支援 AI エージェント。テクニカル分析・相場レジーム判定・ML予測・ポートフォリオ管理・スクリーニングなど 18 コマンドをLINEから操作でき、GitHub Actions で自動ブリーフィング・アラート・レポートを配信します。
...
LINE でエージェントに送信できる全 33 コマンドです。銘柄コードは Yahoo Finance 形式（例: `7203.T`, `^N225`）で指定します。

通知は3階層に分離しています。個人LINEは個人AI秘書、LINEグループは月次ランキングや重大アラートなど「大きなお知らせ」のみ、ツール内チャット(`/wall` `/share` `/comment`)でメンバー間の売買共有・会話を行います。
...
| ポートフォリオ | `/buy <銘柄> <取得価格> <株数> [メモ...]` | 購入を記録 | `/buy 7203.T 3000 100 打診買い` |
| ポートフォリオ | `/sell <銘柄> <株数> [売却価格] [メモ...]` | 売却を記録 | `/sell 7203.T 100 3500 利確` |
...
| グループ共有 | `/group create <名前>` | グループ作成 | `/group create 投資部` |
| グループ共有 | `/group join <コード>` | 招待コードで参加 | `/group join ABC123` |
| グループ共有 | `/group list` | 所属グループ一覧 | `/group list` |
| グループ共有 | `/group members` | 所属グループのメンバー一覧 | `/group members` |
| グループ共有 | `/share <ticker> <buy\|sell> <qty> <price> [pnl] [コメント]` | 売買を共有 | `/share 7203.T buy 100 3200` |
| グループ共有 | `/wall [件数]` | タイムライン閲覧 | `/wall 20` |
| グループ共有 | `/comment <本文>` | コメント投稿 | `/comment 利確しました` |
| グループ共有 | `/ranking [日数]` | グループ内損益ランキング | `/ranking 30` |

### LINEグループ内コマンド

- `/register <名前>` — このLINEグループをSupabaseに登録
- `/ranking [日数]` — 損益ランキング
- `/help` — グループ用ヘルプ
...
| 月次ランキング | 毎月1日 9:00 JST | `scripts/run_group_ranking.py` |
| グループ重大アラート | 平日 11:30 / 15:00 JST | `scripts/run_group_alert.py` |
